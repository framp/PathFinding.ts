...
ThetaStarFinder.prototype.lineOfSight = function (
  startX,
  startY,
  endX,
  endY,
  ...
  if (dx >= dy) {
    ...
    for (s0 = y0, s1 = y1, f = dy; ; ) {
      ...
      if (x0 == x1) {
        if (y0 == y1) {
          if (
            !grid.isWalkableAt(x0, y0 - sy) ||
            !grid.isWalkableAt(x0, y0) ||
            !grid.isWalkableAt(x0, y0 + sy)
          ) {
            return false;
          }
 ...
 *     end positions.
 */
ThetaStarFinder.prototype.findPath = function (
  startX,
  startY,
  endX,
  endY,
  ...
  while (!openList.empty()) {
    ...
    for (i = 0, l = neighbors.length; i < l; ++i) {
      ...
      if (
        node.parent &&
        lineOfSight(x, y, node.parent.x, node.parent.y, grid)
      ) {
...
export default ThetaStarFinder;
